...
3. **The repository root used to contain two generations of self-audit documents** — a stale pre-Sprint-1 set (`01_FINAL_ARCHITECTURE_AUDIT.md` through `05_ARCHITECTURE_FREEZE_DECISION.md`, `ARCHITECTURE_REVIEW.md`, `TECHNICAL_DEBT.md`, `IMPROVEMENT_PLAN.md`, `PROGRESS_REPORT.md`), which said things like "no Wallet/Pricing/Auth/AI-provider code exists" (all false by the time it was still sitting there), alongside the current set (`HANDOVER/`, `FINAL_REVIEW/`, plus this file and its three siblings). **The stale set was deleted during the pre-freeze cleanup pass** (see `ARCHITECTURE_FREEZE.md` and `FINAL_REVIEW/GIT_RELEASE_PLAN.md`) — if you find a reference to one of those filenames anywhere (a handful of now-orphaned citations remain in older commit messages and in `HANDOVER/`'s own text, which describes the state at the time it was written), know that the file no longer exists; it isn't a broken link you need to chase.
