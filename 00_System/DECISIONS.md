...
### D-020 — Aggregator platform workstream now has a concrete home: `Platform/`
**Status:** Provisional
**Decision:** D-011 established that the AIFA aggregator platform (multi-model AI product) is a separate workstream from Content Studio, "not yet started in this repo." That workstream has now started, at `Platform/` — a new top-level folder, sibling to `01_Core`–`10_Orchestrator`, with its own monorepo, architecture docs, and ADR log. It does not merge into, restructure, or reference the Content Studio engine folders, and Content Studio's documentation is unchanged by its existence.
**Rationale:** Founder approved starting the platform workstream and chose to keep it inside this repository (as a sibling folder) rather than a separate repository, for now. This concretizes D-011's "governed elsewhere, once written" without reopening or amending D-011 itself — the scope split it defined is still in force, just no longer describing an empty future.
**Owner:** `Platform/` (self-contained; see `Platform/docs/adr/0001-platform-separation.md` for the detailed rationale and alternatives considered).
**Note:** Everything under `Platform/` is foundation architecture only (repo/monorepo structure, backend/frontend/DB/API architecture, AI provider abstraction, infra, CI/CD) — no business logic (auth, wallet, pricing, provider implementations) per founder-set scope for this milestone.

### D-021 — Company-wide knowledge layer added at `AIFA_Brain/` and `Agents/`, sibling to Content Studio and Platform
**Status:** Provisional
**Decision:** Two new top-level folders, siblings to `01_Core`–`10_Orchestrator` and `Platform/`: `AIFA_Brain/` (product, business, marketing, research, competitors, decisions, learnings, roadmap — a cross-workstream knowledge base referencing both Content Studio and Platform without owning either) and `Agents/` (a framework — standards, communication, shared-memory, and prompt conventions — for future AI agents that don't exist yet; no agent is implemented here). Neither folder merges, restructures, moves, or duplicates content from `00_System`–`10_Orchestrator`/`System/` (Content Studio) or `Platform/` — both are referenced from `AIFA_Brain/12_Decisions/` and `AIFA_Brain/MASTER_INDEX.md` via relative links, never copied.
**Rationale:** Directly mirrors D-020's precedent (a new top-level sibling folder for a new, distinct scope) rather than inventing a new placement pattern. Founder request explicitly asked for a structure supporting "AIFA Platform, AIFA Studio, Future AI Agents... Research, Marketing, Business, Product, Design" as one cross-cutting layer — this is compatible with D-011's separation (which governs the *engineering/content-pipeline* structure of Content Studio specifically, not whether a company-wide business/knowledge layer may exist alongside it) as long as the layer only cross-references, never restructures, either governed workstream. Logged here per the standing rule that anything touching D-011's scope boundary must be recorded, not decided silently.
**Owner:** `AIFA_Brain/` and `Agents/` (self-contained; see `AIFA_Brain/README.md` and `Agents/README.md`).
**Note:** No business/product/marketing/research *content* is fabricated to fill these folders — every folder is real structure (a README stating purpose/update rules/ownership/relationships, and templates where specified) waiting for real content, consistent with this repository's standing "flag gaps, don't guess" discipline (see e.g. D-019's rationale for the same pattern). `Agents/` is framework only — no agent is implemented, per explicit instruction.
**Status note:** Marked Provisional, not Locked, matching D-020's own status — a repository-wide structural addition of this scope should be confirmed by the founder before being treated as permanently settled, the same courtesy D-020 was given.

---

## Change History
- **2026-07-13:** D-021 added — `AIFA_Brain/` (cross-workstream knowledge base) and `Agents/` (future-agent framework, no implementation) added as new top-level sibling folders. Neither Content Studio's nor Platform's existing structure was moved, merged, or altered.
- **2026-07-12:** D-020 added — the AIFA aggregator platform workstream (previously "not yet started" per D-011) now has a concrete home at `Platform/`. Content Studio's structure and decisions are unaffected.
